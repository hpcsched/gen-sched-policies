...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -wfp3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -unicef -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -spt -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -f4 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -f3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -f2 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_ctcsp2.xml -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer) 
  _buffer.close()
...
xoffset=[0.39,0.39,0.39,0.39]
ylow=[1000,1000,1000,1000]
...
for i in range(0,OUT_POLICIES):
  ...
  #  output = '%.1f' % (outliers[i,0])  
    plt.annotate(output, xy=(xticks[i], 1100), xytext=(xticks[i]-xoffset[i], ylow[i]),
...
#plt.show()
plt.savefig('plots/ctc_r.pdf', format='pdf', dpi=1000,bbox_inches='tight')
...
print('Boxplot saved in file plots/ctc_r.pdf')
