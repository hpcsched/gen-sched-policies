...
filename = "swfs/lublin_256_est.swf"
...
print('Performing scheduling performance test for the workload trace lublin_256_est.\n'+
...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -wfp3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -unicef -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -spt -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -f4 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -f3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -f2 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -bf -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  _buffer.close()
...
xoffset=[0.40,0.465,0.40,0.465]
ylow=[6800,6600,6800,7100]
...
for i in range(0,OUT_POLICIES):
  ...
  #if i == 3 or i == 1:
  #  output = '%.1f' % (outliers[i,0])
  plt.annotate(output, xy=(xticks[i], 7700), xytext=(xticks[i]-xoffset[i], ylow[i]),
...
#plt.show()
plt.savefig('plots/model_256_bf.pdf', format='pdf', dpi=1000,bbox_inches='tight')
...
print('Boxplot saved in file model_256_bf.pdf')
