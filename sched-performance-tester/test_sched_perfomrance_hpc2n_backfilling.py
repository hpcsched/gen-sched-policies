...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -wfp3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -unicef -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -spt -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -f4 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -f3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -f2 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_hpc2n.xml -bf -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
...
people = ('FCFS', 'WFP3', 'UNICEF', 'SJF', 'F4', 'F3', 'F2', 'F1')
...
for i in range(0,OUT_POLICIES):
  temp = new_all_data[i,:]
  if i == 0:
    for j in range(0,OUT_CUT):
      _max = np.max(temp)
      #outliers[i,j] = _max
      temp = np.delete(temp,np.argmax(temp))
      out_indices.append(np.argmax(temp)) 
  else:
...
xoffset=[0.32,0.32,0.32,0.32,0.32,0.32,0.32,0.32]
ylow=[210,210,210,210,217,210,210,210]
...
for i in range(0,OUT_POLICIES):
  ...
  #  output = '%.1f' % (outliers[i,0])  
    plt.annotate(output, xy=(xticks[i], 230), xytext=(xticks[i]-xoffset[i], ylow[i]),
...
#plt.show()
plt.savefig('plots/hpc2n_bf.pdf', format='pdf', dpi=1000,bbox_inches='tight')
...
print('Boxplot saved in file hpc2n_bf.pdf')
