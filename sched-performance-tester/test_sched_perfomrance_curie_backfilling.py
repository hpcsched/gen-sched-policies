...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -wfp3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -unicef -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -spt -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -f4 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -f3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -f2 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -bf -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  _buffer.close()
...
for i in range(0,OUT_POLICIES):
  ...
  #  output = '%.1f' % (outliers[i,0])
  if MAX_OUT[i] != 0:
    plt.annotate(output, xy=(xticks[i], 115), xytext=(xticks[i]-xoffset[i], ylow[i]),
...
#plt.show()
plt.savefig('plots/curie_bf.pdf', format='pdf', dpi=1000,bbox_inches='tight')
...
print('Boxplot saved in file curie_bf.pdf')
