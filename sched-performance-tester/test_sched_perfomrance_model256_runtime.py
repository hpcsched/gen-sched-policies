...
print('Performing scheduling performance test for the workload trace lublin_256.\n'+
...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-runtime xmls/plat_day.xml xmls/deployment_day.xml -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
...
for i in range(0,OUT_POLICIES):
  ...
  #else:
  output = '%.1f' % (outliers[i,0])+'\n'+'%.1f' % (outliers[i,1])
  plt.annotate(output, xy=(xticks[i], 17500), xytext=(xticks[i]-0.50, 15000),
            arrowprops=dict(facecolor='black', shrink=0.05),fontsize=27)
...
#plt.show()
plt.savefig('plots/model_256_r_nbf.pdf', format='pdf', dpi=1000,bbox_inches='tight')
...
print('Boxplot saved in file model_256_r.pdf')
