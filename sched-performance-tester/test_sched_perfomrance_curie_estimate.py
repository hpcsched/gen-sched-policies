...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_curie.xml -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  _buffer.close()
...
#print(new_all_data)
plt.plot(xticks[0:2], new_all_data[0:2], 'o', color='darkorange')
...
for i in range(4):
  ...
  if i == 2:
    ...
    plt.annotate(output, xy=(xticks[i], 425), xytext=(xticks[i]-0.45, 390),
            arrowprops=dict(facecolor='black', shrink=0.05),fontsize=35)
  else:
    output = '%.1f' % (outliers[i,0])+'\n'+'%.1f' % (outliers[i,1])
    plt.annotate(output, xy=(xticks[i], 425), xytext=(xticks[i]-0.45, 350),
            arrowprops=dict(facecolor='black', shrink=0.05),fontsize=35)
