...
filename = "swfs/SDSC-BLUE-2000-4.2-cln.swf"
...
print('Performing scheduling performance test for the workload trace SDSC-BLUE-2000-4.2-cln.\n'+
...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -wfp3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -unicef -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -spt -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -f4 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -f3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -f2 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_blue.xml -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  _buffer.close()
...
xoffset=[0.395,0.395,0.32,0.32]
ylow=[2050,2050,2200,2200]
...
for i in range(0,OUT_POLICIES):
  ...
  #  output = '%.1f' % (outliers[i,0])  
    plt.annotate(output, xy=(xticks[i], 2300), xytext=(xticks[i]-xoffset[i], ylow[i]),
...
print('Boxplot saved in file blue_e.pdf')
