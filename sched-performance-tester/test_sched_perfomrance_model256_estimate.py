...
for i in xrange(0,NUM_EXPERIMENTS): #1e7  
  ...
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -wfp3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -unicef -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -spt -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -f4 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -f3 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -f2 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)
  subprocess.call(['./sched-simulator-estimate-backfilling xmls/plat_day.xml xmls/deployment_day.xml -f1 -nt '+str(number_of_tasks)], shell=True, stdout=_buffer)  
  _buffer.close()
...
outliers = np.zeros((OUT_POLICIES,MAX_OUT))

for i in range(0,OUT_POLICIES):
  for j in range(0,MAX_OUT):
    _max = max(all_data[i])
    outliers[i,j] = _max
...
for i in range(0,OUT_POLICIES):
  ...
  #else:
  #output = '%.1f' % (outliers[i,0])+'\n'+'%.1f' % (outliers[i,1])
  plt.annotate(output, xy=(xticks[i], 20000), xytext=(xticks[i]-0.47, 18500),
...
#plt.show()
plt.savefig('plots/model_256_e.pdf', format='pdf', dpi=1000,bbox_inches='tight')
...
print('Boxplot saved in file model_256_e.pdf')
